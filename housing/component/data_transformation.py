...
from housing.util.util import read_yaml_file, load_data,load_numpy_array_data,load_object,save_numpy_array_data, save_object
...
class FeatureGenerator(BaseEstimator, TransformerMixin):
    ...
    def __init__(self,add_bedroom_per_room= True, total_rooms_ix=3,population_ix=5,households_ix=6,total_bedrooms_ix=4, columns=None):
        """
          FeatureGenerator Initialization, add_bedrooms_per_room: bool, total_rooms_ix: int index number of total rooms columns, population_ix: int index number of total population columns
...
class DataTransformation:
    ...
    def get_data_transformer_object(self) -> ColumnTransformer:
        try:
            ...
            num_pipeline= Pipeline(steps=[('imputer', SimpleImputer(strategy="median")),('feature_generator', FeatureGenerator(add_bedroom_per_room=self.data_transformation_config.add_bedroom_per_room,columns=numerical_columns)),('scaler', StandardScaler())])

            cat_pipeline = Pipeline(steps=[('impute', SimpleImputer(strategy="most_frequent")),('one_hot_encoder', OneHotEncoder()),('scaler', StandardScaler(with_mean=False))])
    ...
    def initiate_data_transformation(self)-> DataTransformationArtifact:
        ...
        try:
            ...
            # Assuming target_column_name is ('median_house_value',)
            actual_target_column_name = target_column_name[0]

            input_feature_train_df = train_df.drop(columns=[actual_target_column_name], axis=1)

            target_feature_train_df =  train_df[target_column_name]

            input_feature_test_df= test_df.drop(columns= [actual_target_column_name],axis=1)
            ...
            data_transformation_artifact= DataTransformationArtifact(is_transformed=True,message="Data transformed succesfully",
